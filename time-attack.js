...
const timeResetBtn = document.querySelector(".time-game-reset");
...
function init() {
	...
	timesUpEl.classList.remove("times-over");
	timesUpEl.classList.add("times-up");
}
...
function getTimeAttack() {
	x = setInterval(() => {
		...
		timeRemainingEl.innerText = `Time Remaining: ${timeRemaining}`;
		if (timeRemaining === 0) {
			timesUpEl.classList.remove("times-up");
			timesUpEl.classList.add("times-over");
			clearInterval(x);
		}
	}, 1000);
}
...
function mysteryTheme() {
	let theme = document.getElementById("theme-time");
	if (theme.getAttribute("href") == `./css/timestyle.css`) {
		theme.setAttribute("href", `./css/alttimestyle.css`);
	} else {
		theme.setAttribute("href", `./css/timestyle.css`);
	}
}
